...
> * msg - message to be encoded, given as a list
> * n   - length of the desired encrypted message (or degree of encrypted message polynomial)
> * GF  - finite field over which to encode
...
> * codeword      - encoded message
> * num_of_errors - number of errors to be introduced
> * gf            - finite field over which to introduce errors

Returns a tuple of the codeword with the errors and the number of errors introduced (codeword, error_num)
"""
...
> * codeword      - message to be decoded, given as a list
> * k             - length of the original message
> * num_of_errors - number of errors in the received codeword
> * GF            - finite field over which to decode
...
Return the original message polynomial of degree k - 1 generated by:
 ...
 Returns None if failed to decode
"""
...
> * factors          - list of polynomials f for which to check if f(x_i) = y_i for at least t values
> * xs               - list of x values
> * ys               - list of y values
> * agreement_factor - factor t to check if there is a good agreement
> * k                - maximum degree of factor
...
# # Predicate is_singular_by_x
# ---
#
# ### Parameters:
#
# > * bv_poly - bi-variate polynomial whose ring is over x, y
# > * GF      - finite field the polynomial is over
#
# #### Return true if bv_poly could be represented only by the x variable

# In[6]:


def is_singular_by_x(bv_poly, gf):
    q = gf.characteristic()
    for i in range(q):
        for j in range(q):
            if bv_poly(i, i) != bv_poly(i, j):
                return False
    return True


def get_p_from_factor(factor, gf, ring):
    x, y = ring.gens()
    if is_singular_by_x(factor - y, gf):
        return factor - y
    return None
...
def rs_list_decoder(codeword, k, num_of_errors, gf):
    ...
    for f in list(factors):
        p = get_p_from_factor(f[0], gf, ring)
        if p is not None:
    ...
    if len(factors_list) == 0:
        return None
    return get_agreeing_factors(factors_list, list(range(n)), codeword, t, k)
